...
# CONFIG
# =========================
CHESSBOARD_SIZE = (11, 7)   # nombre de coins horizontaux x verticaux
SQUARE_SIZE = 0.0265           # taille d'une case en mètres (20 mm)
...
def validate_reprojection(image_files, K, dist, rvecs, tvecs, title):
    ...
    while i < len(image_files):
        ...
        for p in corners.reshape(-1,2):
            cv2.circle(vis, tuple(map(int,p)), 4, (0,255,0), -1)
        for p in proj.reshape(-1,2):
            cv2.circle(vis, tuple(map(int,p)), 3, (0,0,255), -1)
...
def main():
    ...
    print(f"Images LEFT trouvées: {len(left_files)}")
    print(f"Images RIGHT trouvées: {len(right_files)}")

    if len(left_files) < MIN_PAIRS or len(right_files) < MIN_PAIRS:
